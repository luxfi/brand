...
> Multi-chain decentralized exchange and the Lux Network

A short, opinionated design spec. Mirrors the spirit of Material 3 / Google
Web Design Spec, sized to the Lux brand. Lives next to `LLM.md` —
`LLM.md` describes what's built, this file describes how it should look.

The canonical brand JSON for Lux is `brand.json` in this repo. That
...
### 1.1 Voice
Confident, technical, financial-grade precision. Numbers and basis points over hype.

### 1.2 Brand atom
Strict monochrome with a subtle warm bias. Black surface, white type. Color reserved for state, market direction, and chain badges.
...
| Token | Value | Use |
|---|---|---|
| `accent1` (dark theme) | `#FFFFFF` | Primary accent |
| `surface1` (dark) | `#000000` | Page surface |
| `surface2` (dark) | `#1A1A1A` | Card surface |
| `neutral1` | `#FFFFFF` | Primary type |
| `neutral2` | `rgba(255,255,255,0.65)` | Secondary type |
...
primary app (lux.exchange). If a page reads visibly different from the
